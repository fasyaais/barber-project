...
const form = useForm<{
    ...
    maps_link: props.data.maps_link,
    img: undefined,
});

function submit() {
    form.put(merchants.update(props.data.id).url, {
        forceFormData: true,
        onSuccess: () => {
            form.reset();
        },
...
<template>
    <div class="space-y-4">
        <div>
            <h1 class="text-2xl font-bold tracking-tight">Edit Gerai</h1>
            <p class="text-sm text-muted-foreground">Perbarui data gerai barbershop.</p>
        </div>
        <Card>
...
<FieldSet>
                            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                                <!-- Nama Gerai -->
                                <Field>
                                    <FieldLabel for="merchant-name"> Nama Gerai </FieldLabel>
                                    <Input
                                        id="merchant-name"
                                        v-model="form.name"
                                    ...
                                    <Popover v-model:open="openComboBox">
                                        <PopoverTrigger as-child>
                                            <Button
                                                ...
                                                :class="cn('w-full justify-between', errors.user_id ? 'border-red-500' : '')"
                                            >
                                                ...
                                                <CommandList>
                                                    ...
                                                    <CommandGroup>
                                                        <CommandItem
                                                            ...
                                                            "
                                                        >
                                                            <CheckIcon :class="cn('mr-2 h-4 w-4', value === user.id ? 'opacity-100' : 'opacity-0')" />
                                                            {{ user.fullname }}
                                                        </CommandItem>
                                                    </CommandGroup>
                                                </CommandList>
                            ...
                            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                                <!-- Alamat Gerai -->
                                <Field>
                                    <FieldLabel for="merchant-address"> Alamat Gerai </FieldLabel>
                                    <Textarea
                                        id="merchant-address"
                                        v-model="form.locate"
                                        placeholder="Masukkan alamat lokasi.."
                                        :class="errors.locate ? 'border-red-500' : ''"
                                    ></Textarea>
                                ...
                                <!-- Link Google Maps -->
                                <Field>
                                    <FieldLabel for="merchant-maps-link"> Link Google Maps </FieldLabel>
                                    <Input
                                        id="merchant-maps-link"
                                        v-model="form.maps_link"
                                        placeholder="https://maps.app.goo.gl/..."
                                        :class="errors.maps_link ? 'border-red-500' : ''"
                                    />
                                    <FieldError v-if="errors.maps_link">
                                        {{ errors.maps_link }}
                                    </FieldError>
                                </Field>
                            </div>
                            ...
                            <!-- Gambar -->
                            <Field>
                                <FieldLabel for="merchant-image"> Gambar </FieldLabel>
                                <Input
                                    id="merchant-image"
                                    type="file"
                                    accept="image/*"
                            ...
                            <Field orientation="horizontal">
                                ...
                                <Link :href="admin.merchants.index()">
                                    <Button variant="destructive" type="button"> Batal </Button>
                                </Link>
                            </Field>
                        </FieldSet>
                    </FieldGroup>
